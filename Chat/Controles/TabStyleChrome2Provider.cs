...
// Type: System.Windows.Forms.TabStyleChrome2Provider
...
{
    [ToolboxItem(false)]
    public class TabStyleChrome2Provider : TabStyleProvider
    {
        public TabStyleChrome2Provider(CustomTabControl tabControl)
          : base(tabControl)
        {
            this._Overlap = 14;
            this._Radius = 16;
        ...
        public override void AddTabBorder(GraphicsPath path, Rectangle tabBounds)
        {
            ...
            if (this._TabControl.Alignment <= TabAlignment.Bottom)
            {
                ...
                num2 = Convert.ToInt32(Math.Truncate(Math.Floor(Decimal.Divide(Decimal.Multiply(new Decimal(tabBounds.Height), Decimal.One), new Decimal(5L)))));
                num3 = Convert.ToInt32(Math.Truncate(Math.Floor(Decimal.Divide(Decimal.Multiply(new Decimal(tabBounds.Height), Decimal.One), new Decimal(5L)))));
                num4 = Convert.ToInt32(Math.Truncate(Math.Floor(Decimal.Divide(Decimal.Multiply(new Decimal(tabBounds.Height), Decimal.One), new Decimal(3L)))));
            }
            else
            {
            ...
            switch (this._TabControl.Alignment)
            {
                case TabAlignment.Top:
                    ...
                    matrix.Translate(50f, 50f);
                    path.Widen(pen, matrix, 1f);
                    GraphicsPath graphicsPath1 = path;
                    Point[] points1 = new Point[4];
                    int index1 = 0;
                    Point point1 = new Point(tabBounds.X, tabBounds.Bottom);
                    ...
                    int index8 = 3;
                    Point point8 = new Point(tabBounds.Right, tabBounds.Bottom);
